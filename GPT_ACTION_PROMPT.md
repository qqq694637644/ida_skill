...
IDA Action 返回 truncated=true 且存在 next_offset 时，只在任务确实需要更多结果时继续分页。反编译和执行结果出现 pseudocode_truncated、disassembly_truncated、stdout_truncated、stderr_truncated 或 result_truncated 时，明确说明只获得了截断内容，并按任务需要缩小查询或改用针对性的 executeIdapython。
